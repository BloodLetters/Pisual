...
use pumpkin_plugin_api::{Server, common::Hand, player::Player, world::World};
    ...
    if !*checked {
        *checked = true;
        if let Some(p) = ctx.player {
            ...
            *cached_player =
                find_nearest_player_snapshot(ctx.server, &holo.world_name, holo.position);
        }
    }
...
fn resolve_placeholder(
    ...
    match key.to_ascii_lowercase().as_str() {
        ...
        "ping" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| player.ping.to_string())
        ...
        "health" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| format!("{:.1}", player.health))
        ...
        "max_health" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| format!("{:.1}", player.max_health))
                    .unwrap_or_else(|| "0.0".to_string()),
            )
        }
        "food" | "food_level" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| player.food.to_string())
                    .unwrap_or_else(|| "0".to_string()),
            )
        }
        "saturation" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| format!("{:.1}", player.saturation))
                    .unwrap_or_else(|| "0.0".to_string()),
            )
        }
        "level" | "exp_level" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| player.level.to_string())
                    .unwrap_or_else(|| "0".to_string()),
            )
        }
        "exp" | "points" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| player.exp.to_string())
                    .unwrap_or_else(|| "0".to_string()),
            )
        }
        "gamemode" => {
        ...
        "item_in_hand" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| player.item_in_hand)
                    .unwrap_or_else(|| "Air".to_string()),
            )
        }
        "player_x" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| format!("{:.2}", player.pos.0))
        ...
        "player_y" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| format!("{:.2}", player.pos.1))
                    .unwrap_or_else(|| "0.00".to_string()),
            )
        }
        "player_z" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| format!("{:.2}", player.pos.2))
                    .unwrap_or_else(|| "0.00".to_string()),
            )
        }
        "player_uuid" | "uuid" => {
            let p = get_or_resolve_player(ctx, cached_player, checked);
            Some(
                p.map(|player| player.uuid)
                    .unwrap_or_else(|| "None".to_string()),
            )
        }
        "online_players" | "online" => Some(
            ctx.server
                .map(|s| s.get_player_count().to_string())
                .unwrap_or_else(|| "0".to_string()),
        ),
        "max_players" | "max" => Some(
            ctx.server
                .map(|s| s.get_max_players().to_string())
                .unwrap_or_else(|| "0".to_string()),
        ),
        "tps" => Some(
            ctx.server
                .map(|s| format!("{:.2}", s.get_tps()))
                .unwrap_or_else(|| "20.00".to_string()),
        ),
        "mspt" => Some(
            ctx.server
                .map(|s| format!("{:.2}", s.get_mspt()))
                .unwrap_or_else(|| "0.00".to_string()),
        ),
        "server_motd" | "motd" => Some(ctx.server.map(|s| s.get_motd()).unwrap_or_default()),
        "server_difficulty" | "difficulty" => Some(
            ctx.server
        ...
        "total_holograms" | "holograms" => Some(
            crate::get_manager()
                .read()
                .map(|mgr| mgr.count().to_string())
                .unwrap_or_else(|_| "0".to_string()),
        ),
        "world" | "world_name" => Some(
            ctx.world
                .map(|w| w.get_id())
        ...
        "world_time" | "time" => Some(
            ctx.world
                .map(|w| w.get_time_of_day().to_string())
                .unwrap_or_else(|| "0".to_string()),
        ),
        "world_age" => Some(
            ctx.world
                .map(|w| w.get_world_age().to_string())
                .unwrap_or_else(|| "0".to_string()),
        ),
        "mc_time" => Some(
            ctx.world
        ...
        "is_raining" => Some(
            ctx.world
                .map(|w| w.is_raining().to_string())
                .unwrap_or_else(|| "false".to_string()),
        ),
        "is_thundering" => Some(
            ctx.world
                .map(|w| w.is_thundering().to_string())
                .unwrap_or_else(|| "false".to_string()),
        ),
        "holo_id" | "hologram_id" => Some(ctx.hologram.map(|h| h.id.clone()).unwrap_or_default()),
        "x" => Some(
            ctx.hologram
                .map(|h| format!("{:.2}", h.position.0))
                .unwrap_or_default(),
        ),
        "y" => Some(
            ctx.hologram
                .map(|h| format!("{:.2}", h.position.1))
                .unwrap_or_default(),
        ),
        "z" => Some(
            ctx.hologram
                .map(|h| format!("{:.2}", h.position.2))
                .unwrap_or_default(),
        ),
        "line_count" => Some(
            ctx.hologram
                .map(|h| h.lines.len().to_string())
                .unwrap_or_else(|| "0".to_string()),
        ),
        "billboard" => Some(
            ctx.hologram
                .map(|h| format!("{:?}", h.billboard))
                .unwrap_or_default(),
        ),
        "view_range" => Some(
            ctx.hologram
                .map(|h| format!("{:.1}", h.view_range))
                .unwrap_or_default(),
        ),
        _ => None,
    }
    ...
    while let Some(start_idx) = remaining.find('{') {
        ...
        if let Some(end_idx) = after_start.find('}') {
            ...
            if !candidate_key.contains('{') {
                if let Some(val) =
                    resolve_placeholder(candidate_key, ctx, &mut cached_player, &mut checked)
                {
                    result.push_str(&val);
                } else {
                    result.push('{');
